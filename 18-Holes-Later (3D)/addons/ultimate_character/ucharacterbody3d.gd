...
func _input(event):
	if !Engine.is_editor_hint() and first_person:
	...
	if !Engine.is_editor_hint():
		...
		if is_on_floor():
			...
			if first_person:
				...
				direction = lerp(direction, (Vector3(input_dir.x, 0, input_dir.y)).normalized(), delta * 10.0)
				var dir = Vector2(direction.x, direction.z)
				rotation.y = -dir.angle() - deg_to_rad(90)
				#rotate_y(deg_to_rad(-event.relative.x * MOUSE_SENSITIVITY))
				#head_node.rotate_x(deg_to_rad(-event.relative.y * MOUSE_SENSITIVITY))
			...
			if input_dir != Vector2.ZERO:
				if first_person:
					...
					direction = lerp(direction, (Vector3(input_dir.x, 0, input_dir.y)).normalized(), delta * 3.0)
